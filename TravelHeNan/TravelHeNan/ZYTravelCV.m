...
@interface ZYTravelCV ()<UITableViewDataSource,UITableViewDelegate>
@property (nonatomic, retain) UITableView *bigTableView;

@end

@implementation ZYTravelCV
...
-(void)setRequestHttp
{
    [CYHttpRequest requestGetOneCompletion:^(NSArray *dataArray) {
        [_dataArray addObject:dataArray];
        [CYHttpRequest requestGetTwoCompletion:^(NSArray *dataArray) {
            [_dataArray addObject:dataArray];
            [CYHttpRequest requestGetThirdCompletion:^(NSArray *dataArray) {
                [_dataArray addObject:dataArray];
...
{
    CYDetail1ViewController *detailVC = [[CYDetail1ViewController alloc]init];
    detailVC.sort = _sort;
    detailVC.row = (int)indexPath.row;
    detailVC.section = _select;
